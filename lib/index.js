...
class Capture {
  ...
  startRecord () {
    this.exporter.start().then(() => {
      this.active = true;
      this.recordData.started = this.project.elapsedTime;
  ...
  stopRecord () {
    this.active = false;
    this.recordData.ended = this.project.elapsedTime;
    ...
    // we send the audio to the exporter 
    if (this.options.audio) {
      let blob = this.options.audio.manager.getAudioCopy(this.recordData.ended, this.recordData.ended-this.recordData.started);
      this.exporter.addAudioTrack(blob);
    }
