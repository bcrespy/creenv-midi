...
class Capture {
  ...
  constructor (project, options) {
    ...
    if (this.options.controls) {
      this.controls = new CaptureControls();
    }
  ...
  /**
   * render a frame, given the correct time input, 
   */
  renderNewFrame () {
    if (!this.done) {
      // we check if duration is not exceeded
      if (this.active && this.options.duration && this.fakeElapsedTime-this.recordData.started > this.options.duration) {
        this.stopRecord();
      }

      this.project.elapsedTime = this.fakeElapsedTime;
      this.project.deltaT = this.fakeElapsedTime;
      this.project.render();
