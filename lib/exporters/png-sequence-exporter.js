...
const getFilename = (prefix, index) => {
  let suffix = "0000000"+index;
  return prefix+suffix.substr(suffix.length-7, 7)+".png";
}
...
class PNGSequenceExporter extends Exporter {
  ...
  /**
   * generates an url pointing to the blob @param blob and then triggers click on that url, which will generate a download
   * 
   * @param {Blob} blob 
   */
  blobDownload (blob) {
