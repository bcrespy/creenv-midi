...
class CaptureControls {
  constructor (onPlay, onPause, onRecord, onStop, onPrev, onNext, onCapture, framerate = false, infoBoxRecord = true, buttonInfos = true, duration = false) {
  ...
  /**
   * updates the graphics to match a pause
   */
  pause () {
    this.dom.getElementsByClassName("play")[0].classList.remove("paused");
  }
  ...
  createDOM () {
    ...
    if (this.buttonInfos) this.dom.classList.add("button-infos");
    this.dom.innerHTML = `
      ...
      <div class="play-buttons">
        <div class="button change-frame previous" data-infos="pevious frame"></div>
        <div class="button play paused" data-infos="play/pause"></div>
        <div class="button change-frame next" data-infos="next frame"></div>
      </div>
      <div class="record-buttons">
        <div class="button screenshot" data-infos="export current frame as png"></div>
        <div class="button record" data-infos="start/stop recording"></div>
      </div>`;
  ...
  setupEvents () {
    ...
    this.dom.getElementsByClassName("screenshot")[0].addEventListener("click", () => {
      this._onCapture();
    });
